//you can replace all the text in the engine through this list

//globals
add_string("deaths","Deaths")
add_string("time","Time")

//volume check
add_string("volcheck","Volume Check#")
add_string("volcheck2"," to confirm")
add_string("volcheck3","Use ")
add_string("volcheck4"," and ")

//ingame text
add_string("warptohub","UP to return")
add_string("pausemenu","PAUSE")
add_string("presstosave"," to Save")
add_string("signpost","Poggers")
add_string("hitcount","Hits taken: ")

//menus
add_string("menuback","Back")
add_string("menuoptions","Options")
add_string("menuaccept","Accept")

//file menu
add_string("filemenu","- Select Your Saved Game -")
add_string("filenodata","No Data")
add_string("filegame","Game ")
add_string("fileclear","Clear!")
add_string("filediff","Difficulty:")
add_string("filecontinue","Continue")
add_string("fileerase","Erase Game")
add_string("fileundo","Press Ctrl-Z to Undo")

//vsync ui
add_string("vsynctest","vSync testing")
add_string("vsynctestfail","vSync test failed!")
add_string("vsynctestok","vSync OK!")

//joystick ui
add_string("joyfound","New joystick found:")
add_string("joysfound"," new joysticks found:")
add_string("joyset1up","Go to Options to set it up")
add_string("joyset2up","Go to Options to set them up")
add_string("joyusekey","Please use your keyboard here")
add_string("joypushtosel","Push a button to select joystick")
add_string("joytokey","Please disable JoyToKey and#Steam Generic Controller support")


//options menu
add_string("optionsmenu","- Options -")
add_string("optionsitem1","Music Volume")
add_string("optionsitem2","Sound Volume")
add_string("optionsitem3","Screen Mode")
add_string("optionsitem4","VSync")
add_string("optionsitem5","Fullscreen Filter")
add_string("optionsitem6","Sprite Filter")
add_string("optionsitem7","Blood Effect")
add_string("optionsitem8","Left+Right Mode")
add_string("optionsitem9","Keyboard Setup")
add_string("optionsitem10","Joystick Setup")
add_string("optionswindow0","Window")
add_string("optionswindow1","Fullscreen")
add_string("optionsvsync0","Failed")
add_string("optionsvsync1","Off")
add_string("optionsvsync2","On")
add_string("optionsvsynctest","Testing...")
add_string("optionsflip0","Flip")
add_string("optionsflip1","Stay")
add_string("optionsflip2","Stop")
add_string("optionsflip3","Left")
add_string("optionsflip4","Right")
add_string("optionsmooonwalk","Moonwalk")
add_string("optionsfilter0","Off")
add_string("optionsfilter1","Bilinear")
add_string("optionsfilter2","Area")
add_string("optionssmooth0","Off")
add_string("optionssmooth1","On")
add_string("optionsblood0","Off")
add_string("optionsblood1","Low")
add_string("optionsblood2","Medium")
add_string("optionsblood3","High")
add_string("optionsnojoys","no joys")
add_string("optionsjoys"," joys")

//keyboard setup
add_string("keymenu","- Control Setup -")
add_string("keyname0","Left")
add_string("keyname1","Right")
add_string("keyname2","Up" )
add_string("keyname3","Down")
add_string("keyname4","Jump")
add_string("keyname5","Shoot")
add_string("keyname6","Menu")
add_string("keyname7","Restart")
add_string("keyname8","Skip")
add_string("keyname9","Die")
add_string("keyresetkeys","Reset Controls")
add_string("keynewkey","Press new key...")
add_string("keykeysreset","Reset!")
