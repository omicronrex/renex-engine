...
if (argument0=="step") {
    ...
    repeat (30) if (wait_timer(4)) {
        ...
        instance_create_moving(x,y,CherryYellow,4*dt,point_direction(x,y,xx,yy))
    }
    wait_timer(8)
    repeat (32) if (wait_timer(4)) {
        xx=Player.x+random_range(-128,128)
        yy=Player.y+random_range(-128,128)
        instance_create_moving(x,y,CherryYellow,6*dt,point_direction(x,y,xx,yy))
        instance_create_moving(x+120,y,CherryYellow,6*dt,point_direction(x,y,xx,yy))
    }
    ...
    repeat (2) if (wait_timer(4)) {
        instance_create_moving(x,y,MikuBigCherryBlue,6*dt,random_range(135,225))
    }
    ...
    if (wait_timer(32)) {
        with (CherryMagenta) {
            instance_create_moving(x,y,CherryGray,10*dt,180)
            instance_destroy()
        }
    }
    if (wait_timer(50)) {
